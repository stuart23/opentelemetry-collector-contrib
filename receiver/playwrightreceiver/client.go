...
// sendMessage sends a Playwright message without waiting for a response
func (c *PlaywrightClient) sendMessage(ctx context.Context, guid string, method string, params interface{}, metadata interface{}) error {
	c.mu.Lock()
	if c.conn == nil || c.closed {
		c.mu.Unlock()
	...
	if params != nil {
		if pm, ok := params.(map[string]interface{}); ok {
			...
			return fmt.Errorf("params must be map[string]interface{} or map[string]string")
		}
	}
	...
	if metadata != nil {
		if mm, ok := metadata.(map[string]interface{}); ok {
			...
			return fmt.Errorf("metadata must be map[string]interface{}")
		}
	}
	...
	c.mu.Unlock()
	if err != nil {
		return fmt.Errorf("failed to send Playwright message: %w", err)
	}

	return nil
}

// sendCommand sends a Playwright command and waits for response
func (c *PlaywrightClient) sendCommand(ctx context.Context, guid string, method string, params interface{}, metadata interface{}) (*PlaywrightResponse, error) {
	c.mu.Lock()
	if c.conn == nil || c.closed {
		c.mu.Unlock()
...
func (c *PlaywrightClient) Initialize(ctx context.Context) (*InitializeResult, error) {
	...
	// Send the initialize message using sendMessage
	err := c.sendMessage(ctx, "", "initialize", params, metadata)
	if err != nil {
	...
	// Listen for messages until we get the initialize response
	...
	for {
		...
		case resp := <-initRespChan:
			messageCount++
			if result := c.checkInitializeResponse(resp, messageCount, state); result != nil {
				return result, nil
			}
		case <-initCtx.Done():
			return nil, fmt.Errorf("initialize timeout after 15s, received %d messages: %w", messageCount, initCtx.Err())
		case <-c.closeChan:
			return nil, fmt.Errorf("connection closed during initialize")
		}
	}
}
...
func (c *PlaywrightClient) NewBrowserCDPSession(ctx context.Context, browserGUID string) (*NewBrowserCDPSessionResult, error) {
	...
	resp, err := c.sendCommand(ctx, browserGUID, "newBrowserCDPSession", nil, metadata)
	if err != nil {
...
func (c *PlaywrightClient) sendCDPMessage(ctx context.Context, sessionGUID string, cdpMethod string, cdpParams interface{}, metadata interface{}) (*PlaywrightResponse, error) {
	...
	// Send the command using sendCommand method with "send" as the method and wait for response
	resp, err := c.sendCommand(ctx, sessionGUID, "send", wrappedParams, metadata)
	if err != nil {
